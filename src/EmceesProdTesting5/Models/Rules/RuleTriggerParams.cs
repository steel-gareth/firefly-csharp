...
/// </summary>
public record class RuleTriggerParams : ParamsBase
{
    ...
    /// applied to. If the end date is not present, it will be set to today. If you
    /// use this field, both the start date and the end date must be present.
    /// </summary>
    public string? End
    {
        get
    ...
    /// <summary>
    /// A date formatted YYYY-MM-DD, to limit the transactions the actions will be
    /// applied to. If the start date is not present, it will be set to one year
    /// ago. If you use this field, both the start date and the end date must be present.
    /// </summary>
    public string? Start
    {
        get
    ...
    public RuleTriggerParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public RuleTriggerParams(RuleTriggerParams ruleTriggerParams)
        : base(ruleTriggerParams)
    {
        this.ID = ruleTriggerParams.ID;
    }
...
#pragma warning restore CS8618
    ...
    public static RuleTriggerParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        string id
    )
    ...
    public virtual bool Equals(RuleTriggerParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/v1/rules/{0}/trigger", this.ID)
        )
        {
