...
/// </summary>
public record class RuleTestParams : ParamsBase
{
    public string? ID { get; init; }

    /// <summary>
    /// Limit the testing of the rule to these asset accounts or liabilities. Only
    /// asset accounts and liabilities will be accepted. Other types will be silently dropped.
    /// </summary>
    ...
    public RuleTestParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public RuleTestParams(RuleTestParams ruleTestParams)
        : base(ruleTestParams)
    {
        this.ID = ruleTestParams.ID;
    }
...
#pragma warning restore CS8618
    ...
    public static RuleTestParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        string id
    )
    ...
    public virtual bool Equals(RuleTestParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + string.Format("/v1/rules/{0}/test", this.ID)
        )
        {
