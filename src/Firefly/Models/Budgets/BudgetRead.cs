...
[JsonConverter(typeof(JsonModelConverter<BudgetRead, BudgetReadFromRaw>))]
public sealed record class BudgetRead : JsonModel
{
...
#pragma warning disable CS8618
    [SetsRequiredMembers]
    public BudgetRead(BudgetRead budgetRead)
        : base(budgetRead) { }
#pragma warning restore CS8618

    public BudgetRead(IReadOnlyDictionary<string, JsonElement> rawData)
    {
...
#pragma warning disable CS8618
    [SetsRequiredMembers]
    BudgetRead(FrozenDictionary<string, JsonElement> rawData)
    {
        this._rawData = new(rawData);
    }
#pragma warning restore CS8618

    /// <inheritdoc cref="BudgetReadFromRaw.FromRawUnchecked"/>
    public static BudgetRead FromRawUnchecked(IReadOnlyDictionary<string, JsonElement> rawData)
    {
        return new(FrozenDictionary.ToFrozenDictionary(rawData));
    }
}

class BudgetReadFromRaw : IFromRawJson<BudgetRead>
{
    /// <inheritdoc/>
    public BudgetRead FromRawUnchecked(IReadOnlyDictionary<string, JsonElement> rawData) =>
        BudgetRead.FromRawUnchecked(rawData);
}
...
public sealed record class Attributes : JsonModel
{
    ...
    public string? AutoBudgetAmount
    {
        ...
        {
            this._rawData.Freeze();
            return this._rawData.GetNullableClass<string>("auto_budget_amount");
        }
        init { this._rawData.Set("auto_budget_amount", value); }
    }
    ...
    public ApiEnum<string, AutoBudgetPeriod>? AutoBudgetPeriod
    {
        ...
        {
            this._rawData.Freeze();
            return this._rawData.GetNullableClass<ApiEnum<string, AutoBudgetPeriod>>(
                "auto_budget_period"
            );
        }
        init { this._rawData.Set("auto_budget_period", value); }
    }
    ...
    public ApiEnum<string, AutoBudgetType>? AutoBudgetType
    {
        ...
        {
            this._rawData.Freeze();
            return this._rawData.GetNullableClass<ApiEnum<string, AutoBudgetType>>(
                "auto_budget_type"
            );
        }
        init { this._rawData.Set("auto_budget_type", value); }
    }
    ...
    /// </summary>
    public string? PcAutoBudgetAmount
    {
        get
        {
            this._rawData.Freeze();
            return this._rawData.GetNullableClass<string>("pc_auto_budget_amount");
        }
        init { this._rawData.Set("pc_auto_budget_amount", value); }
    }

    /// <summary>
    /// Information on how much was spent in this budget. Is only filled in when
    /// the start and end date are submitted. It is converted to the primary currency
    ...
    /// <summary>
    /// Information on how much was spent in this budget. Is only filled in when
    /// the start and end date are submitted.
    /// </summary>
    ...
    public override void Validate()
    {
        _ = this.Name;
        _ = this.Active;
        _ = this.AutoBudgetAmount;
        this.AutoBudgetPeriod?.Validate();
        this.AutoBudgetType?.Validate();
        _ = this.CreatedAt;
        _ = this.CurrencyCode;
