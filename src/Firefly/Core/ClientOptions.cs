...
public record struct ClientOptions()
{
    ...
    Lazy<string> _baseUrl = new(() =>
        Environment.GetEnvironmentVariable("FIREFLY_BASE_URL") ?? EnvironmentUrl.Production
    );
    ...
    /// <see cref="IFireflyClient.WithRawResponse"/>, don't perform validation until deserialization
    /// is triggered.</para>
    /// </summary>
    ...
    Lazy<string?> _bearerToken = new(() =>
        Environment.GetEnvironmentVariable("FIREFLY_BEARER_TOKEN")
    );
