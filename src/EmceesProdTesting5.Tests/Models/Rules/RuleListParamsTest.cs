...
public class RuleListParamsTest : TestBase
{
    ...
    {
        RuleListParams parameters = new() { Limit = 10, Page = 1 };

        var url = parameters.Url(new() { });

        Assert.True(
            TestBase.UrisEqual(
                new Uri("https://demo.firefly-iii.org/api/v1/rules?limit=10&page=1"),
                url
            )
        );
    }
    ...
    public void AddHeadersToRequest_Works()
    {
        ...
        RuleListParams parameters = new() { XTraceID = "40c71bbb-c676-4f24-83cf-cc725d7d7a00" };
